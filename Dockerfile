...
RUN apt update -y
RUN apt install -y \
    bison cmake curl ffmpeg git git-lfs imagemagick kmod openjdk-21-jre openssh-server \
...
RUN apt install -y \
    ...
    libxext-dev libfreetype-dev libgtk-3-dev libjpeg-dev libopenal-dev libpng-dev \
    libdecor-0-dev libgbm-dev libwayland-dev libxkbcommon-x11-dev \
    linux-libc-dev libusb-1.0-0-dev libcap2-bin libffi-dev libssl-dev \
    libxi6 libreadline-dev
RUN apt install -y \
